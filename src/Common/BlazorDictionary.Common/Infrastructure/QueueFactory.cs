...
namespace BlazorDictionary.Common.Infrastructure
{
    public static class QueueFactory
    {
        public static void SendMessageToExchange(string exchangeName, string exchangeType, string queueName, object obj)
        {
            ...
            channel.BasicPublish(exchangeName, routingKey: queueName, basicProperties:null, body: body);
        }
        ...
        public static EventingBasicConsumer EnsureQueue(this EventingBasicConsumer consumer, string queueName, string exchangeName)
        {
            ...
            consumer.Model.QueueBind(queue: queueName, exchange: exchangeName, queueName); //route key ile queue name aynı olur. O yüzden key yerine queue name yazdık.
